...
function rgba(red: number, green: number, blue: number, alpha: number): string {
  return (
    'rgba(' +
    Math.round(red * 255).toString() +
    ',' +
    Math.round(green * 255).toString() +
    ',' +
    Math.round(blue * 255).toString() +
    ',' +
    alpha.toString() +
    ')'
  );
...
export function convertKnownToolToAnchorTag(
  ...
  KNOWN_TOOLS.forEach((tool) => {
    value = value.replace(
      new RegExp(tool, 'g'),
      addAnchorTag(tool, run, sessionPath, runPath),
    );
  });
  return value;
}
...
export function parseDiagnosticsDataTable(
  diagnosticsTable: SimpleDataTable | null,
): Diagnostics {
...
export function setLoadingState(
  ...
  if (loading) {
    ...
    message =
      message + '. First-time session loads may take up to a few minutes.';
  }
  store.dispatch(
