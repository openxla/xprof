...
export class HloStats extends Dashboard implements OnDestroy {
  ...
  private mergeTables(
    ...
    const programIdCol = active.cols.findIndex((col) => col.id === PROGRAM_ID);
    const categoryCol = active.cols.findIndex(
      (col) => col.id === OP_CATEGORY_ID,
    );
    const nameCol = active.cols.findIndex((col) => col.id === OP_NAME_ID);
    const exprCol = active.cols.findIndex((col) => col.id === OP_EXPRESSION_ID);
    const coreTypeCol = active.cols.findIndex((col) => col.id === CORE_TYPE_ID);

    const totalTimeCol = active.cols.findIndex(
      (col) => col.id === TOTAL_TIME_ID,
    );
    const selfTimeCol = active.cols.findIndex((col) => col.id === SELF_TIME_ID);
    const flopRateCol = active.cols.findIndex(
      (col) => col.id === MEASURED_FLOP_RATE_ID,
    );
    const occurrencesCol = active.cols.findIndex(
      (col) => col.id === OCCURRENCES_ID,
    );
    const avgTimeCol = active.cols.findIndex((col) => col.id === AVG_TIME_ID);
    ...
    const aligned = alignTables(active.rows, baseline.rows, getRowKey);
    ...
    const baselineOccurrencesColIndex = occurrencesCol !== -1 ? newCols.length : -1;
    if (occurrencesCol !== -1) {
      newCols.push({
      ...
      if (activeRow) {
        ...
        if (baselineTotalTimeColIndex !== -1) {
          newCells[baselineTotalTimeColIndex] = {
            v: baselineRow?.c?.[totalTimeCol]?.v ?? 0,
            f: baselineRow?.c?.[totalTimeCol]?.f,
          };
        }
        if (baselineSelfTimeColIndex !== -1) {
          newCells[baselineSelfTimeColIndex] = {
            v: baselineRow?.c?.[selfTimeCol]?.v ?? 0,
            f: baselineRow?.c?.[selfTimeCol]?.f,
          };
        }
        if (baselineFlopRateColIndex !== -1) {
          newCells[baselineFlopRateColIndex] = {
            v: baselineRow?.c?.[flopRateCol]?.v ?? 0,
            f: baselineRow?.c?.[flopRateCol]?.f,
          };
        }
        if (baselineOccurrencesColIndex !== -1) {
          newCells[baselineOccurrencesColIndex] = {
            v: baselineRow?.c?.[occurrencesCol]?.v ?? 0,
            f: baselineRow?.c?.[occurrencesCol]?.f,
          };
        }
        if (baselineAvgTimeColIndex !== -1) {
          newCells[baselineAvgTimeColIndex] = {
            v: baselineRow?.c?.[avgTimeCol]?.v ?? 0,
            f: baselineRow?.c?.[avgTimeCol]?.f,
          };
        }
