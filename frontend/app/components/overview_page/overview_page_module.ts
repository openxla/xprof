...
import {type GeneralAnalysis, type InputPipelineAnalysis, type NormalizedAcceleratorPerformance, type OverviewPageDataTuple, type RunEnvironment, type SimpleDataTable} from 'org_xprof/frontend/app/common/interfaces/data_table';
...
export class OverviewPage implements OnDestroy {
  ...
  @Output() readonly onRunEnvUpdate = new EventEmitter<RunEnvironment|null>();
  @Output()
  readonly onNormalizedAcceleratorPerformanceUpdate =
      new EventEmitter<NormalizedAcceleratorPerformance|null>();
  ...
  parseOverviewPageData(data: OverviewPageDataTuple) {
    ...
    this.onRunEnvUpdate.emit(this.runEnvironment);
    this.onNormalizedAcceleratorPerformanceUpdate.emit(
