...
export class SideNav implements OnInit, OnDestroy {
  ...
  get isTraceViewerPending() {
    const tag = this.selectedTagInternal || '';
    return tag === 'trace_viewer' || tag === 'trace_viewer@';
  }
  ...
  async onTagSelectionChange(tag: string) {
    ...
    const isChangingToTraceViewer =
        !previousSelectedTag ||
        (this.isTraceViewerPending && previousSelectedTag !== this.selectedTag);

    this.selectedHostsInternal = [];
    this.selectedHostsPending = [];
    this.selectedHostInternal = '';

    if (isChangingToTraceViewer) {
  ...
  updateUrlHistory() {
    ...
    // --- START Replacement for createUrlTree().toString() ---
    ...
    // --- END Replacement for createUrlTree().toString() ---
