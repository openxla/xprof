...
export class SourceMapper implements OnDestroy, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['opName'] &&
      ...
      if (!this.compilerPasses.includes(this.selectedCompilerPass)) {
        this.selectedCompilerPass = this.compilerPasses[0] as CompilerPass;
      }
      this.update();
    }
  ...
  update() {
    ...
    this.maybeUpdateHloUnoptimizedTextCache();
  ...
  maybeUpdateHloUnoptimizedTextCache() {
    if (!this.programId || this.programId === '0' || this.sessionId === '' ||
        this.selectedCompilerPass !== CompilerPass.HLO_UNOPTIMIZED) {
      return;
    }
    const hloText = this.hloUnoptimizedTextByProgramId.get(this.programId);
    if (hloText) {
      return;
    }
    this.dataService
        ...
        .subscribe((data) => {
          if (data) {
            this.hloUnoptimizedTextByProgramId.set(
                this.programId, data as string);
          }
        });
  }
