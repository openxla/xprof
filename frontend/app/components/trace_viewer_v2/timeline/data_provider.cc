...
bool CompareThreadsForSort(ProcessId pid, const TraceInformation& trace_info,
  ...
  const auto it_name_a = trace_info.thread_names.find({pid, a});
  const auto it_name_b = trace_info.thread_names.find({pid, b});
  const bool has_name_a = (it_name_a != trace_info.thread_names.end());
  const bool has_name_b = (it_name_b != trace_info.thread_names.end());

  if (has_name_a && has_name_b) {
    if (it_name_a->second != it_name_b->second) {
      return it_name_a->second < it_name_b->second;
    }
  } else if (has_name_a != has_name_b) {
    return has_name_a;
  }
...
std::vector<ProcessId> GetSortedProcessIds(const TraceInformation& trace_info) {
  ...
  absl::c_stable_sort(pids, [&](ProcessId a, ProcessId b) {
    ...
    const auto it_name_a = trace_info.process_names.find(a);
    const auto it_name_b = trace_info.process_names.find(b);
    const bool has_name_a = (it_name_a != trace_info.process_names.end());
    const bool has_name_b = (it_name_b != trace_info.process_names.end());
