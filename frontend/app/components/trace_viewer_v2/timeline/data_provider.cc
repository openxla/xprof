...
void DataProvider::ProcessTraceEvents(
    absl::Span<const TraceEvent> event_list, Timeline& timeline,
    std::optional<TimeRange> full_time_span) {
  if (replace_events_) {
    timeline.set_data_time_range({std::numeric_limits<double>::max(),
                                  std::numeric_limits<double>::lowest()});
  }
  if (event_list.empty()) {
    if (full_time_span.has_value()) {
      timeline.set_data_time_range(
        {std::min(timeline.data_time_range().start(), full_time_span->start()),
         std::max(timeline.data_time_range().end(), full_time_span->end())});
    }
    return;
  }
  ...
  for (const TraceEvent& event : event_list) {
  ...
  if (full_time_span.has_value()) {
    ...
    timeline.set_data_time_range(
        {std::min(timeline.data_time_range().start(), time_bounds.min),
         std::max(timeline.data_time_range().end(), time_bounds.max)});
  }
  ...
  if (!initial_viewport_was_set_) {
    Microseconds view_start = 0;
    if (full_time_span.has_value()) {
      view_start = full_time_span->start();
    } else if (has_events) {
      view_start = time_bounds.min;
    }
    timeline.SetVisibleRange({view_start, view_start + 1000000},
                             /*animate=*/true);
...
std::optional<EventMetaData> DataProvider::GetEventMetaData(
  ...
  metadata.processName = process_it == process_names_.end()
                               ? GetDefaultProcessName(event->pid)
                               : process_it->second;
...
bool DataProvider::IsEventLoadedOrAdd(const TraceEvent& event) {
  if (loaded_event_hashes_.contains(event.hash_key)) {
    return true;
  }
  loaded_event_hashes_.insert(event.hash_key);
  return false;
}

void DataProvider::ClearLoadedEventHashes() {
  loaded_event_hashes_.clear();
  initial_viewport_was_set_ = false;
}
