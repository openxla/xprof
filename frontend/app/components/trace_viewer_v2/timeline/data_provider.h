...
class DataProvider {
 public:
  ...
  void ProcessTraceEvents(
      ...
      std::optional<TimeRange> full_time_span = std::nullopt);
  ...
  bool IsEventLoadedOrAdd(const TraceEvent& event);
  void ClearLoadedEventHashes();
  void set_replace_events(bool replace_events) {
    replace_events_ = replace_events;
  }
 ...
 private:
  ...
  bool replace_events_ = true;
  absl::flat_hash_set<std::string> loaded_event_hashes_;
};
