...
void Timeline::MaybeRequestData() {
  ...
  // Refetch data if:
  // 1. The preserved range is not fully contained in the currently fetched data
  //    (user scrolled out of range).
  // 2. The user zoomed in significantly, making the current data resolution too
  //    coarse (fetched range is much larger than the needed fetch range).
