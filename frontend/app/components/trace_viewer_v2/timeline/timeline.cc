...
void Timeline::MaybeRequestData() {
  ...
  if (!data_time_range_.Contains(preserve)) {
    ...
    EventData event_data;
    event_data.try_emplace(std::string(kRequestDataStart), fetch.start());
    event_data.try_emplace(std::string(kRequestDataEnd), fetch.end());

    event_callback_(kRequestData, event_data);
    is_loading_ = true;
  }
}
