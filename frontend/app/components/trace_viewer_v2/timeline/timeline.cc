...
void Timeline::DrawMinimap(Pixel content_region_avail_width) {
  ...
  draw_list->AddLine(
      ImVec2(full_bg_min.x, full_bg_max.y),
  ...
  draw_list->AddLine(
      ImVec2(divider_x, full_bg_min.y),
  ...
  const bool show_duration_badge =
      label_width_ > duration_badge_w + 80.0f;
  ...
  if (show_duration_badge) {
    ...
    draw_list->AddRect(ImVec2(badge_x1, badge_y1),
                       ImVec2(badge_x2, badge_y2),
  ...
  const Pixel max_title_w =
      show_duration_badge
          ? (divider_x - kSplitterOffset - duration_badge_w - 16.0f -
             (minimap_screen_pos.x + kIndentSize))
  ...
  ImGui::SetCursorScreenPos(
      ImVec2(minimap_screen_pos.x + kIndentSize, text_y));
  ...
  if (parent_idx >= 0 &&
    ...
    const int num_bins =
        static_cast<int>(std::ceil(current_timeline_width_));
    if (num_bins > 0) {
      ...
      for (int i = 0; i < num_bins; ++i) {
        if (minimap_bins_[i] > 0.0f) {
          ...
          h = std::max(1.0f, h);
          draw_list->AddRectFilled(
              ImVec2(track_x1 + i, track_y2 - 2.0f - h),
  ...
  if (lens_x1 > track_x1) {
    draw_list->AddRectFilled(
        ImVec2(track_x1, track_y1),
        ImVec2(lens_x1, track_y2),
        kMinimapMaskColor);
  }
  if (lens_x2 < track_x2) {
    draw_list->AddRectFilled(
        ImVec2(lens_x2, track_y1),
        ImVec2(track_x2, track_y2),
        kMinimapMaskColor);
  }

  // Draw Draggable Viewport Selection Lens
  draw_list->AddRectFilled(
      ImVec2(lens_x1, track_y1),
      ImVec2(lens_x2, track_y2),
      kMinimapLensColor);
  ...
  draw_list->AddLine(
      ImVec2(lens_x1, track_y1 + 4.0f),
      ImVec2(lens_x1, track_y2 - 4.0f),
      kWhiteColor, 1.0f);
  draw_list->AddLine(
      ImVec2(lens_x2, track_y1 + 4.0f),
      ImVec2(lens_x2, track_y2 - 4.0f),
      kWhiteColor, 1.0f);
  ...
  if (lens_width > 24.0f) {
    ImGui::PushClipRect(
        ImVec2(lens_x1 + handle_hw + 1.0f, track_y1),
...
void Timeline::HandleMinimapInteraction(
    Pixel track_x1, Pixel track_x2, Pixel track_y1, Pixel track_y2,
    const TimeRange& total_range, Pixel lens_x1, Pixel lens_x2) {
  const ImVec2 mouse_pos = ImGui::GetMousePos();
  const Pixel handle_hw = kMinimapHandleWidth * 0.5f;

  const bool mouse_over_track = mouse_pos.x >= track_x1 &&
                                mouse_pos.x <= track_x2 &&
                                mouse_pos.y >= track_y1 &&
  ...
  const bool mouse_over_lens = mouse_pos.x >= lens_x1 &&
                              mouse_pos.x <= lens_x2 &&
                              mouse_pos.y >= track_y1 &&
                              mouse_pos.y <= track_y2;

  // Set appropriate mouse cursor
  if (minimap_drag_mode_ == MinimapDragMode::kResizeLeft ||
  ...
  if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
    if (mouse_over_left_handle) {
      ...
      // Click outside lens: center lens on clicked position
      ...
      // Transition immediately into pan dragging
      minimap_drag_mode_ = MinimapDragMode::kPan;
      minimap_drag_start_x_ = mouse_pos.x;
      minimap_initial_visible_range_ = visible_range();
    }
  ...
  if (ImGui::IsMouseDown(ImGuiMouseButton_Left) &&
    ...
    if (minimap_drag_mode_ == MinimapDragMode::kPan) {
      ...
      Microseconds new_start =
          minimap_initial_visible_range_.start() + delta_time;
      const Microseconds min_end =
          visible_range().end() - kMinDurationMicros;
      ...
      Microseconds new_end =
          minimap_initial_visible_range_.end() + delta_time;
