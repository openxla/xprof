...
class TimelineImGuiTestFixture : public Test {
 protected:
  ...
  void SimulateLabelColumnResizeDragStart() {
    ...
    float resize_handle_y = win_y + 34.0f;
...
TEST_F(RealTimelineImGuiFixture, DrawSelectedTimeRangeTextAtCorrectYPosition) {
  ...
  // So expected text_y is 0 + 24.0f + 5.0f = 29.0f.
  // Adding the observed 3.0f ImGui font vertical offset gives 32.0f.
...
TEST_F(RealTimelineImGuiFixture, RulerHeightAndProcessHeaderAlignment) {
  EXPECT_FLOAT_EQ(kRulerHeight, 24.0f);
  ...
  // Verify Tracks child window Y position starts at kRulerHeight relative to
  // ruler start.
  const float ruler_start_screen_y = timeline_window->DC.CursorStartPos.y;
  EXPECT_FLOAT_EQ(tracks_window->Pos.y - ruler_start_screen_y, kRulerHeight);

  // Verify horizontal dividing line position at ruler_start_screen_y +
  // kRulerHeight.
  const float expected_line_y = ruler_start_screen_y + kRulerHeight;
  bool found_dividing_line = false;
  for (const auto& vtx : timeline_window->DrawList->VtxBuffer) {
    if (vtx.col == kRulerLineColor &&
        std::abs(vtx.pos.y - expected_line_y) <= 1.0f) {
      found_dividing_line = true;
      break;
    }
  }
  EXPECT_TRUE(found_dividing_line)
      << "Horizontal dividing line not found at expected Y: "
      << expected_line_y;

  // Verify (kRulerHeight - text_height) * 0.5f vertical centering.
  ImGui::PushFont(traceviewer::fonts::label_large);
  const Pixel text_height = ImGui::GetTextLineHeight();
  ImGui::PopFont();
  const Pixel expected_vertical_offset = (kRulerHeight - text_height) * 0.5f;

  const ImU32 text_col = ImGui::GetColorU32(ImGuiCol_Text);
  float min_header_text_y = std::numeric_limits<float>::max();
  bool found_header_text = false;
  for (const auto& vtx : timeline_window->DrawList->VtxBuffer) {
    if (vtx.col == text_col &&
        vtx.pos.y >= ruler_start_screen_y &&
        vtx.pos.y < ruler_start_screen_y + kRulerHeight) {
      if (vtx.pos.y < min_header_text_y) {
        min_header_text_y = vtx.pos.y;
      }
      found_header_text = true;
    }
  }
  EXPECT_TRUE(found_header_text) << "Process header text not found in ruler";
  EXPECT_NEAR(
      min_header_text_y - ruler_start_screen_y, expected_vertical_offset,
      3.0f);
...
TEST_F(TimelineDragSelectionTest, SnapIncludesEventsAtExactBottomEdgeOfWindow) {
  ...
  // event (which is at 24.0f) is exactly at the bottom visible edge of the
...
TEST_F(TimelineMouseModeSelectTestSuite, FindSelectedEventsEmitsJson) {
  ...
  // Set start position.
  io.MousePos = ImVec2(GetTimelineStartX() + 200.0f, 39.0f);
  io.AddMouseButtonEvent(0, true);
...
TEST_F(RealTimelineImGuiFixture, CollapseAllHeaderHidesGroups) {
  ...
  // "Hidden" header is at 0-30 (screen 24-54)
  // "Pinned" header is at 30-60 (screen 54-84)
  // "All" header is at 60-90 (screen 84-114)
  // Button is at X = kIndentSize (10), Y ~ 99
  io.MousePos = ImVec2(15.0f, 99.0f);
  SimulateFrame();
...
TEST_F(RealTimelineImGuiFixture, ExpandHiddenHeaderShowsHiddenGroups) {
  ...
  // "Hidden" header is at 0-30 (screen 24-54)
  // Button is at X = kIndentSize (10), Y ~ 39
  io.MousePos = ImVec2(15.0f, 39.0f);
  SimulateFrame();
...
TEST_F(RealTimelineImGuiFixture, ClickUnhideButtonOnHiddenTrackUnhidesIt) {
  ...
  // Hidden is at 0-30 local, 24-54 screen.
  io.MousePos = ImVec2(15.0f, 39.0f);
  SimulateFrame();
  io.AddMouseButtonEvent(0, true);
  SimulateFrame();
  ...
  // (e.g. 30 + 25 = 55 local, + 24 = 79 screen).
  io.MousePos = ImVec2(241.0f, 79.0f);
  SimulateFrame();
...
TEST_F(RealTimelineImGuiFixture, MinimapDragSelectionJumpsFlameChartViewport) {
  FlameChartTimelineData data;
  data.groups = {
      MakeProcessGroup("Process 1", /*start_level=*/0, /*level_count=*/1),
  };
  ...
  const Pixel track_x1 = timeline_.GetLabelWidth();
  const Pixel track_w = timeline_.current_timeline_width();
  const ImVec2 start_pos(track_x1 + 0.1f * track_w, 14.0f);
  const ImVec2 end_pos(track_x1 + 0.6f * track_w, 14.0f);

  // Mouse down at 10% (100us)
  ImGui::GetIO().MousePos = start_pos;
  SimulateFrame();
  ImGui::GetIO().MouseDown[0] = true;
  SimulateFrame();

  EXPECT_EQ(timeline_.minimap_drag_mode_for_test(),
            Timeline::MinimapDragMode::kSelectRange);

  // Drag to 60% (600us)
  ImGui::GetIO().MousePos = end_pos;
  SimulateFrame();

  EXPECT_TRUE(timeline_.minimap_is_selecting_range());

  // Mouse release: jumps flame chart visible range to [100us, 600us]
  ImGui::GetIO().MouseDown[0] = false;
  SimulateFrame();

  EXPECT_NEAR(timeline_.visible_range().start(), 100.0, 5.0);
  EXPECT_NEAR(timeline_.visible_range().end(), 600.0, 5.0);
  EXPECT_EQ(timeline_.minimap_drag_mode_for_test(),
            Timeline::MinimapDragMode::kNone);
}

TEST_F(RealTimelineImGuiFixture, MinimapDragSelectionRightToLeft) {
  FlameChartTimelineData data;
  data.groups = {
      MakeProcessGroup("Process 1", /*start_level=*/0, /*level_count=*/1),
  };
  ...
  const Pixel track_x1 = timeline_.GetLabelWidth();
  const Pixel track_w = timeline_.current_timeline_width();
  const ImVec2 start_pos(track_x1 + 0.7f * track_w, 14.0f);
  const ImVec2 end_pos(track_x1 + 0.3f * track_w, 14.0f);

  // Mouse down at 70% (700us)
  ImGui::GetIO().MousePos = start_pos;
  SimulateFrame();
  ImGui::GetIO().MouseDown[0] = true;
  SimulateFrame();

  // Drag backwards to 30% (300us)
  ImGui::GetIO().MousePos = end_pos;
  SimulateFrame();

  EXPECT_TRUE(timeline_.minimap_is_selecting_range());

  // Mouse release: jumps viewport to [300us, 700us]
  ImGui::GetIO().MouseDown[0] = false;
  SimulateFrame();

  EXPECT_NEAR(timeline_.visible_range().start(), 300.0, 5.0);
  EXPECT_NEAR(timeline_.visible_range().end(), 700.0, 5.0);
}

TEST_F(RealTimelineImGuiFixture, MinimapDragSelectionShiftKeyInsideLens) {
  FlameChartTimelineData data;
  data.groups = {
      MakeProcessGroup("Process 1", /*start_level=*/0, /*level_count=*/1),
  };
  ...
  timeline_.SetTimelineData(std::move(data));
  timeline_.set_data_time_range({0.0, 1000.0});
  timeline_.SetVisibleRange({200.0, 600.0}, /*animate=*/false);
  timeline_.set_minimap_enabled(true);
  ...
  const Pixel track_x1 = timeline_.GetLabelWidth();
  const Pixel track_w = timeline_.current_timeline_width();
  const ImVec2 start_pos(track_x1 + 0.3f * track_w, 14.0f);
  const ImVec2 end_pos(track_x1 + 0.5f * track_w, 14.0f);

  // Hold Shift key and drag inside the lens [200us, 600us]
...
TEST_F(RealTimelineImGuiFixture, MinimapDragSelectionInSelectModeInsideLens) {
  FlameChartTimelineData data;
  data.groups = {
      MakeProcessGroup("Process 1", /*start_level=*/0, /*level_count=*/1),
  };
  ...
  // In Select mode, drag inside the lens [200us, 600us] initiates range select
  ImGui::GetIO().MousePos = start_pos;
  SimulateFrame();
  ImGui::GetIO().MouseDown[0] = true;
  SimulateFrame();
...
TEST_F(RealTimelineImGuiFixture, MinimapDragSelectionClampsToTotalRange) {
  FlameChartTimelineData data;
  data.groups = {
      MakeProcessGroup("Process 1", /*start_level=*/0, /*level_count=*/1),
  };
  ...
  const Pixel track_x1 = timeline_.GetLabelWidth();
  const Pixel track_w = timeline_.current_timeline_width();
  const ImVec2 start_pos(track_x1 + 0.8f * track_w, 14.0f);
  // Drag past the right edge
  const ImVec2 end_pos(track_x1 + 1.2f * track_w, 14.0f);
  ...
  EXPECT_NEAR(timeline_.visible_range().start(), 800.0, 5.0);
  EXPECT_NEAR(timeline_.visible_range().end(), 1000.0, 5.0);
}

TEST_F(RealTimelineImGuiFixture, MinimapDragSelectionEnforcesMinDuration) {
  FlameChartTimelineData data;
  data.groups = {
      MakeProcessGroup("Process 1", /*start_level=*/0, /*level_count=*/1),
  };
  ...
  const Pixel track_x1 = timeline_.GetLabelWidth();
  const Pixel track_w = timeline_.current_timeline_width();
  const ImVec2 start_pos(track_x1 + 0.5f * track_w, 14.0f);
  // Drag by 5 pixels (exceeds 3px threshold, but corresponds to small time)
  const ImVec2 end_pos(start_pos.x + 5.0f, 14.0f);
  ...
  EXPECT_GE(timeline_.visible_range().duration(), kMinDurationMicros);
}
