...
TEST_F(RealTimelineImGuiFixture, DrawRulerRendersProperly) {
  ...
  if (!found_trace_vertical_line) {
    std::cout << "DEBUG: draw list vtx count=" << draw_list->VtxBuffer.Size
              << std::endl;
    for (const auto& vtx : draw_list->VtxBuffer) {
      std::cout << "vtx.col=" << std::hex << vtx.col << " pos=(" << vtx.pos.x
                << "," << vtx.pos.y << ")\n";
    }
    std::cout << "kTraceVerticalLineColor=" << std::hex
              << kTraceVerticalLineColor << "\n";
  }

  ASSERT_TRUE(found_trace_vertical_line)
      << "Failed to find vertical trace lines";
