...
ParsedTraceEvents ParseCompressedTraceEvents(
  ...
  tensorflow::profiler::TraceDataResponse response;
  if (!response.ParseFromString(*decompressed_proto)) {
    return result;
  }

  // 1. Process Metadata events
  ...
  for (const auto& series : response.async_events()) {
    ...
    for (int i = 0; i < series.deltas_size(); ++i) {
      ...
      if (ev_meta.flow_id() != 0) {
        ...
        // In legacy JSON, FLOW_START/FLOW_END async events had no duration.
        // We only generate horizontal slices for async events with non-zero
        // duration to avoid polluting the arrow geometry engine with 0-length
        // line segments.
