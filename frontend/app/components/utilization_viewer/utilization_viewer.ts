...
const BANDWIDTH_CHART_OPTIONS_BASELINE: google.visualization.BarChartOptions = {
  ...
  height: 1050,
  chartArea: {...BANDWIDTH_CHART_OPTIONS.chartArea, height: 950},
  colors: ['#1a73e8', '#adc6fa'],
  legend: {position: 'top'},
};
...
const DELTA_THRESHOLD = 0.5 * Math.pow(10, -DISPLAY_PRECISION); // 0.005

declare interface NodeChartDataInfoMap {
  [index: number]: ChartDataInfo;
}

/** Interface representing delta information between active and baseline metrics. */
...
/** Computes the percentage difference, formatted delta string, and color. */
export function getDeltaInfo(activePct: number, basePct: number): DeltaInfo {
  const diffPct = activePct - basePct;
  if (Math.abs(diffPct) < DELTA_THRESHOLD) {
    return {
      diffPct: 0,
      formattedDelta: (0).toFixed(DISPLAY_PRECISION) + '%',
      color: '#70757a',
    };
  } else if (diffPct > 0) {
    return {
      diffPct,
      formattedDelta: `+${diffPct.toFixed(DISPLAY_PRECISION)}%`,
      color: '#137333',
    };
  } else {
    return {
...
export class UtilizationViewer extends Dashboard implements OnDestroy {
  ...
  override parseData(data: SimpleDataTable | null) {
    ...
    if (dataTable) {
      ...
      const hasBaseline =
        !!this.diffService.getBaseSessionId() &&
