...
bool IsDerivedFromWeight(const xla::HloInstruction* inst,
                         std::set<const xla::HloInstruction*>& visited) {
  ...
  for (const auto* operand : inst->operands()) {
    if (!IsDerivedFromWeight(operand, visited)) {
      return false;
    }
  }
  return true;
...
std::string ExtractJaxVariablePath(absl::string_view tf_op) {
  ...
  auto shard_map_it = std::find(parts.begin(), parts.end(), "shard_map");
  ...
  for (size_t i = 0; i < parts.size(); ++i) {
    if (RE2::FullMatch(parts[i], kFuncPattern)) {
      return absl::StrJoin(parts.begin() + i + 1, parts.end(), "/");
    }
  }
...
std::pair<std::string, std::string> CategorizeTensor(
    ...
    const std::set<std::string>& gathered_weights) {
  ...
  if (!label_parts.empty()) {
    std::string inst_name(absl::StripAsciiWhitespace(label_parts[0]));
    if (gathered_weights.find(inst_name) != gathered_weights.end()) {
...
absl::StatusOr<std::set<std::string>> TraceGatheredWeights(
  ...
  std::set<const xla::HloInstruction*> gathered_weights;
  std::set<const xla::HloInstruction*> visited;
  ...
  for (const auto* computation : module->computations()) {
    for (const auto* inst : computation->instructions()) {
      if (inst->opcode() == xla::HloOpcode::kAllGather) {
        std::set<const xla::HloInstruction*> backward_visited;
  ...
  while (!q.empty()) {
    auto state = q.front();
    q.pop();
    const auto* inst = state.inst;
    const auto* calling_fusion = state.calling_fusion;
    ...
    if (inst == inst->parent()->root_instruction() &&
      ...
      for (const auto* user : calling_fusion->users()) {
    ...
    for (const auto* user : inst->users()) {
      if (user->opcode() == xla::HloOpcode::kFusion) {
        const auto* called_comp = user->called_computations()[0];
        for (int i = 0; i < user->operand_count(); ++i) {
          if (user->operand(i) == inst) {
            const auto* param = called_comp->parameter_instruction(i);
  ...
  std::set<std::string> gathered_weight_names;
  for (const auto* inst : gathered_weights) {
...
absl::StatusOr<std::string> ConvertHloProtoToMemoryAnalysisJson(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      tensorflow::profiler::ConvertHloProtoToPreprocessResultJson(hlo_proto));

  nlohmann::json preprocessed_json =
      nlohmann::json::parse(preprocessed_json_str);

  std::set<std::string> gathered_weights;
  auto gathered_weights_or = TraceGatheredWeights(hlo_proto);
  if (gathered_weights_or.ok()) {
    gathered_weights = std::move(gathered_weights_or).value();
  } else {
    LOG(WARNING) << "Failed to trace gathered weights: "
                 << gathered_weights_or.status().message();
  }

  std::set<int64_t> memory_colors;
  ...
  for (int64_t color : memory_colors) {
    ...
    if (preprocessed_json.contains("maxHeap")) {
      for (const auto& buffer : preprocessed_json["maxHeap"]) {
        ...
        std::string instruction = buffer.value("instructionName", "");
