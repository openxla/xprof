...
export class MemoryTreemap implements OnChanges {
  ...
  private reconstructZoomStackForPath(path: string) {
    ...
    for (const segment of pathSegments) {
      const child = curr.children.find((c) => c.name === segment);
      if (child) {
        newStack.push(child);
        curr = child;
      } else {
  ...
  render() {
    ...
    const activeChildren = this.currentRoot.children.filter((c) => c.value > 0);
  ...
  private addRect(node: TreeNode, x: number, y: number, w: number, h: number) {
    ...
    if (node.children && node.children.length > 0) {
      ...
      if (w > padding * 2 + 10 && h > padding * 2 + 16 + 10) {
        ...
        const activeChildren = node.children.filter((c) => c.value > 0);
        if (activeChildren.length > 0) {
          this.squarify(
            activeChildren,
            [],
