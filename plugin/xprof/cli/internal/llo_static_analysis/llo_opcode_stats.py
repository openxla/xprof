...
ISA opcode catalog, not in this proto, so the category column must not be
treated
as ground truth.
"""
...
# Enum value -> name, resolved from the opcode field descriptor so we do not
# need to import the (differently named) opcode py_pb2 module directly.
_OPCODE_VALUES = (
    llo_instruction_pb2.LloInstructionProto.DESCRIPTOR.fields_by_name[
        "opcode"
...
def render_opcode_stats_markdown(module, top_n: int = 30) -> str:
  """Renders the opcode histogram + coarse category breakdown as markdown."""
  buf = io.StringIO()
  buf.write(f"# Opcode stats: {module.hlo_instruction_name}\n\n")
  hist = opcode_histogram(module)
  if not hist:
    buf.write("_No LLO instructions found in this module._\n")
    return buf.getvalue()

  total = sum(hist.values())
  buf.write(f"_Total instructions: {total}_\n\n")

  cats = category_histogram(module)
  buf.write("## Coarse category breakdown (heuristic)\n\n")
  buf.write("| Category | Count | Share |\n|---|---:|---:|\n")
  for cat in sorted(cats, key=lambda c: -cats[c]):
    pct = round(100 * cats[cat] / total)
    buf.write(f"| {cat} | {cats[cat]} | {pct}% |\n")

  buf.write("\n## Top opcodes\n\n")
  buf.write("| Opcode | Count | Share |\n|---|---:|---:|\n")
  for name in sorted(hist, key=lambda n: (-hist[n], n))[:top_n]:
    pct = round(100 * hist[name] / total)
    buf.write(f"| `{name}` | {hist[name]} | {pct}% |\n")
  return buf.getvalue()
