...
def _add_bundle_label(
    by_bundle: dict[int, list[str]], bundle: int, label: str
) -> None:
  ...
  for loc in source_map.locations:
    ...
    for ordinal in loc.ordinals:
      if ordinal in ord_to_bundle:
        _add_bundle_label(by_bundle, ord_to_bundle[ordinal], label)
        ...
        if 0 <= start < limit:
          ...
          for b in range(start, clamped_limit):
            _add_bundle_label(by_bundle, b, label)
  return by_bundle


def render_source_for_bundle(
    module: llo_lite_pb2.LloModuleProto, bundle: int
) -> str:
  """Renders the source frame(s) associated with `bundle` as JSON."""
...
def render_bundles_for_source(
    module: llo_lite_pb2.LloModuleProto, source_query: str
) -> str:
  """Renders the bundles whose source frame matches `source_query` as JSON."""
  by_bundle = build_source_by_bundle(module)
  ...
  for label, bundles in sorted(matched.items()):
    ...
    matches.append({
        "source": label,
        "bundles": bundles,
        "start_bundle": bundles[0],
        "limit_bundle": bundles[-1] + 1,
    })
  payload = {
      "hlo_instruction_name": module.hlo_instruction_name,
