...
def get_kernel_stats(
  ...
  ProfileData/XSpace objects. When include_summary=True (or return_dict=True
  ...
  Args:
    ...
    return_dict: If True, returns raw records (or enriched dict when
      include_summary is also True) instead of string.
  ...
  try:
    ...
    if not kernel_durations_us:
      ...
      if return_dict and include_summary:
        return {
...
def get_avg_step_time(
  ...
  try:
    ...
    if not step_durations_ms:
      ...
      return res_err if return_dict else json.dumps(res_err, indent=2)
    ...
    return res if return_dict else json.dumps(res, indent=2)
