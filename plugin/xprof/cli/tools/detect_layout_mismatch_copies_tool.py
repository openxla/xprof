...
# Safety cap on the number of distinct (instruction, shape-index) states a
# single BFS traversal may expand. Traversal state is keyed on (id, shape_idx)
# to precisely follow tuple-element dataflow; on graphs with very wide tuples
# threaded through nested control flow (e.g. a large optimizer-state tuple
# carried by a training-step while loop) the number of live shape_idx contexts
# can blow up combinatorially within max_depth hops. This cap bounds
# worst-case work per copy. It is set far above what any normal copy needs
# (typical copies expand only tens to hundreds of states), so it only ever
# truncates pathological traversals. Because a copy is flagged as a bottleneck
# as soon as a single upstream producer and a single downstream compute stage
# are reachable -- both of which are found within the first few hops --
# capping the exhaustive enumeration does not change which copies are
# detected.
_MAX_BFS_STATES = 50000
...
def _get_id(obj: Any) -> Any:
  """Returns the id of an HLO instruction proto, or a raw id if given one."""
...
def _get_name(obj: Any) -> str:
  """Gets the string name of an HLO instruction proto, or a raw name."""
...
def _get_operands(instr: Any) -> Sequence[Any]:
  """Gets the operand ids for a Protobuf HloInstructionProto."""
  return instr.operand_ids


def _get_opcode_lower(instr: Any) -> str:
  """Gets the lowercased opcode string for a Protobuf HloInstructionProto."""
  # Protobuf opcodes are already lowercase, hyphenated strings
  # (e.g. "copy", "custom-call", "get-tuple-element"); lower() is idempotent.
  return instr.opcode.lower()
...
def is_compute_stage(
  ...
  if memo is not None and instr_id in memo:
  ...
  if any(keyword in opcode_lower for keyword in _COMPUTE_KEYWORDS):
    ...
    result = is_compute_custom_call(instr)
  elif opcode_lower == "fusion":
    if instr_id in visited_fusions:
      return False
    visited_fusions.add(instr_id)

    called_comp_names = [
        comp_by_id[cid].name
        for cid in instr.called_computation_ids
...
def find_upstream_compute_stages(
    ...
    opcode_by_id: Mapping[int, str] | None = None,
    called_comp_ids_by_id: Mapping[int, Sequence[int]] | None = None,
    max_depth: int = 5,
    memo: dict[int, bool] | None = None,
  ...
  Args:
    ...
    callers_by_comp_id: A mapping from computation IDs to their caller
      instruction IDs.
    opcode_by_id: Optional precomputed mapping from instruction ID to its
      lowercased opcode string. Built internally if not provided.
    called_comp_ids_by_id: Optional precomputed mapping from instruction ID to
      the list of computation IDs it calls. Built internally if not provided.
  ...
  """
  if opcode_by_id is None:
    opcode_by_id = {i: instr.opcode.lower() for i, instr in instr_by_id.items()}
  if called_comp_ids_by_id is None:
    called_comp_ids_by_id = {
        i: list(instr.called_computation_ids)
        for i, instr in instr_by_id.items()
    }
  ...
  while queue:
    if len(visited) > _MAX_BFS_STATES:
      break
    ...
    if dist > 0:
      ...
      if opcode == "parameter" and (curr_comp_id not in callers_by_comp_id):
        upstream_producers.append((curr_instr, dist))
        continue
      ...
      if opcode == "parameter":
        ...
        for caller_id in callers:
          ...
          if caller_opcode == "conditional":
            ...
            body_comp_ids = called_comp_ids_by_id.get(caller_id, [])
            if len(body_comp_ids) > 1:
              body_comp_id = body_comp_ids[1]
        ...
        if len(curr_called) > 1:
          body_comp_id = curr_called[1]
          body_comp = comp_by_id.get(body_comp_id)
          if body_comp and body_comp.root_id:
            if (body_comp.root_id, shape_idx) not in visited:
              visited.add((body_comp.root_id, shape_idx))
      ...
      if curr_id == root_id:
        ...
        for caller_id in callers:
          ...
          if caller_opcode == "while":
            caller_called = called_comp_ids_by_id.get(caller_id, [])
            if caller_called:
              if len(caller_called) > 1:
                body_comp_id = caller_called[1]
              else:
                body_comp_id = caller_called[0]
              body_comp = comp_by_id.get(body_comp_id)
              if body_comp:
                for inner_i in body_comp.instructions:
                  if (
                    ...
                    inner_id = inner_i.id
                    if (inner_id, shape_idx) not in visited:
                      visited.add((inner_id, shape_idx))
                      queue.append((inner_id, shape_idx, dist + 1))
        ...
        for user_id in users_by_id.get(curr_id, []):
          ...
          if not user_instr:
            continue
          if opcode_by_id.get(user_id, "") == "get-tuple-element":
        ...
        for user_id in users_by_id.get(curr_id, []):
          ...
          if not user_instr:
            continue
          if opcode_by_id.get(user_id, "") == "tuple":
            operands = user_instr.operand_ids
...
def detect_layout_mismatch_copies(
  ...
  Raises:
    FileNotFoundError: If no HLO proto is found in the session.
  ...
  try:
    ...
    has_copy_in_metrics = any(
        "copy" in k[1].lower() or "copy" in k[0].lower() for k in op_metrics
    )
    if op_metrics and not has_copy_in_metrics:
      ...
      logging.info(
          ...
          "Fetch HLO Proto time: 0.00s, "
          "Fetch Top Ops time: %.2fs, "
          "Parse Top Ops time: %.2fs, "
          "Dict build time: 0.00s, "
          "Copy scan time: 0.00s, "
          "BFS time (0 copies): 0.00s, "
          "Report gen time: 0.00s, "
          "Sort & Format time: 0.00s",
          session_id,
          total_wall_time_s,
    ...
    if not debug_info.hlo_proto:
      raise FileNotFoundError(f"No HLO proto found in session {session_id}.")
    ...
    for hlo_proto in debug_info.hlo_proto:
      ...
      for comp in module_proto.computations:
        for instr in comp.instructions:
          ...
          opcode_by_id[instr_id] = instr.opcode.lower()
          called_ids = list(instr.called_computation_ids)
          called_comp_ids_by_id[instr_id] = called_ids
          for called_comp_id in called_ids:
      ...
      for instr in instr_by_id.values():
        ...
        opcode = opcode_by_id[instr.id]
        if opcode != "copy":
          copy_scan_time_total += time.time() - copy_scan_start
          continue

        operands = instr.operand_ids
        if not operands:
          copy_scan_time_total += time.time() - copy_scan_start
          continue

        operand_id = operands[0]
        ...
        instr_id = instr.id
        instr_comp_id = comp_id_by_instr_id.get(instr_id)
        instr_comp_name = (
            comp_name_by_id.get(instr_comp_id, "") if instr_comp_id else ""
        )
        instr_name_str = instr.name
        metrics = (
        ...
        # Short-circuit the AND gate below: a copy only qualifies as a
        # bottleneck if it has BOTH an upstream producer and a downstream
        # compute stage. When there is no upstream producer the copy can never
        # qualify, so skip the (potentially expensive) downstream BFS entirely.
        if not upstream_producers:
        ...
        if upstream_producers and downstream_stages:
          ...
          recommendation_parts.append(
              ...
              " producers (compute, parameters, or constants)"
    ...
    logging.exception(
        "Error detecting layout mismatch copy operations for Session ID: %s",
        session_id,
    )
    raise RuntimeError(
        f"Error detecting layout mismatch copy operations: {e}"
    ) from e
