...
def detect_unfused_updates(
  ...
  try:
    ...
    if "error" in ops_data:
      ...
      if "Unexpected data type" in err_msg or "NoneType" in err_msg:
        raise FileNotFoundError(
            ...
            f" request failed or timed out for session {session_id})."
        )
      raise ValueError(f"Backend error in top HLO ops: {err_msg}")
    ...
    logging.exception(
        "Error detecting unfused updates for Session ID: %s", session_id
    )
    raise RuntimeError(
        f"Error detecting unfused updates for session {session_id}: {e}"
    ) from e
