...
_LOW_PRECISION_TYPES = frozenset({_BF16_TYPE, _F16_TYPE})
_FP8_TARGET_TYPES = frozenset({
    _F8E5M2_TYPE,
    _F8E4M3FN_TYPE,
    _F8E4M3B11FNUZ_TYPE,
    _F8E5M2FNUZ_TYPE,
...
# Defensive traversal cap against malformed/cyclic HLO.
_MAX_TRAVERSAL_NODES = 10000
...
class _HloModuleTracer:
  ...
  def trace_upstream_activation(
    ...
    while stack:
      count += 1
      if count > _MAX_TRAVERSAL_NODES:
        # Defensive cap against malformed/cyclic HLO; bail out safely.
        return (None, ())
      ...
      if opcode == "get-tuple-element":
        ...
        if instr.called_computation_ids:
          ...
          # Cycle guard: never re-enter a fusion already on the call stack.
          if comp and instr.id not in call_stack:
    ...
    while stack:
      ...
      if comp and curr_id == comp.root_id:
        ...
        if call_stack:
          ...
          fusion_caller = (
              self.fusion_callers.get(comp_id) if comp_id is not None else None
          )
          if fusion_caller:
...
def analyze_hlo_module(
  ...
  return list({b["name"]: b for b in bottlenecks}.values())
...
def detect_unnecessary_convert_dynamic_scale(
  ...
  Raises:
      FileNotFoundError: If HLO proto cannot be found.
      ValueError: If inputs or JSON formats are invalid.
  ...
  try:
    ...
    logging.exception(
        "Failed to fetch top ops for candidate lookup for Session ID: %s",
        session_id,
    )
    raise RuntimeError(f"Failed to fetch top ops: {e}") from e
  ...
  try:
    ...
    logging.exception(
        "Failed to fetch debug info for Session ID: %s", session_id
    )
    raise RuntimeError(f"Failed to fetch debug info: {e}") from e
  fetch_hlo_proto_end_time = time.time()

  if (
      not debug_info
      or not hasattr(debug_info, "hlo_proto")
      or not debug_info.hlo_proto
  ):
