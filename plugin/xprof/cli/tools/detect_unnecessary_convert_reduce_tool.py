...
def detect_unnecessary_convert_reduce(
  ...
  Raises:
      FileNotFoundError: If top HLO ops or HLO debug info cannot be fetched.
      ValueError: If backend data format is unexpected or malformed.
  ...
  try:
    ...
    logging.exception(
        "Error detecting reduce convert overhead for Session ID: %s",
        session_id,
    )
    raise RuntimeError(
        f"Error detecting reduce convert overhead for session {session_id}: {e}"
    ) from e
