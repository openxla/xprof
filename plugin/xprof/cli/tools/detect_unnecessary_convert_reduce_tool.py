...
class HloModuleTracer:
  ...
  def classify_context(self, instr: Any) -> str:
    ...
    for name_str in names_lower:
      if any(kw in name_str for kw in loss_keywords):
        return "LOSS"

    norm_keywords = {"norm", "variance", "mean"}
    for name_str in names_lower:
      if any(kw in name_str for kw in norm_keywords):
        return "NORM"

    for name_str in names_lower:
      if "softmax" in name_str:
        return "SOFTMAX"
  ...
  def trace_upcast(
    ...
    for op_id in instr.operand_ids:
      ...
      if op_instr and op_instr.opcode.lower() == "constant":
        continue
      res = self.trace_upcast(op_id, tuple_index, visited)
      if res is not None:
        return res
  ...
  def trace_downcast(
      ...
      start_instr_id: int,
      tuple_index: int | None = None,
      prev_instr_id: int | None = None,
      visited: set[tuple[int, int | None]] | None = None,
  ) -> bool:
  ...
  try:
    ...
    for op in top_ops:
      ...
      if full_mod_name not in tracers:
        tracers[full_mod_name] = HloModuleTracer(module_proto)
      ...
      if target_instr is None:
        continue
      assert target_instr is not None
