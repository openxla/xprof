...
def _logging_wrapper(
  ...
  """WSGI middleware to log request timing in GKE-friendly JSON format.
  ...
  def wrapper(
    ...
    def custom_start_response(
      ...
      try:
        ...
        logger.warning('Failed to parse status code from %r', status)
...
class ProfilePlugin(base_plugin.TBPlugin):
  ...
  def get_plugin_apps(
    ...
    return {route: _logging_wrapper(app) for route, app in apps.items()}  # pytype: disable=bad-return-type
