...
### When Input Distribution Changes a Numerical Verdict — and When It Does Not
...
> [!NOTE] **Dynamic Range Reach & Mathematical Thresholds**: Empirical
> measurements show standard generated `float32` suites reach peak values around
> $|x| \approx 59.36$. While this stresses exponent variation and dynamic range
> substantially better than standard Gaussians ($[-3, 3]$), it does not
> naturally reach float32 $\exp(x)$ overflow ($x \ge 88.7228$, where
> $88.7228 \approx \ln(\text{FLT\_MAX})$) or FP8 e4m3 saturation ($448.0$)
> without explicit operator-specific target scaling.
>
> **Softmax Underflow vs. Overflow Thresholds**: Note that $88.7228$ is the
> **overflow** threshold ($\ln(\text{FLT\_MAX})$). In float32 Softmax,
> subnormals begin when logits drop below $x \approx -87.34$
> ($\exp(-87.34) \approx 1.17 \times 10^{-38}$), and flush-to-zero (FTZ) occurs
> when $x \le -104.0$ ($\exp(-104.0) \equiv 0.0$). Failing to normalize logits
> ($x - \max(x)$) triggers FTZ when max difference exceeds $104$.
