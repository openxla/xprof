...
# OperationLevelAnalysis.updateAndDrawPieCharts() in Roofline Model populates
# its DefaultDataProvider (this.vl.Db(...)) and assigns its custom data
# processor (this.eja.Jg = new m4(...)) without calling notifyCharts()
# (this.vl.update.emit()), unlike updateAndDrawScatterChart() on the line
# below it. When Google Charts is already warm from an earlier waypoint,
# google.charts.setOnLoadCallback fires before /data?tag=roofline_model
# returns; once the response arrives, Db() populates dataTable without
# emitting update, leaving <chart charttype="PieChart"> blank. Scheduling
# this.update.emit() on the microtask whenever dataTable is assigned on a
# dataProvider (unless update.emit() already fired synchronously in the same
# turn) guarantees the pie chart draws without double-drawing charts that
# already emit synchronously.
_CHART_DATA_PROVIDER_INIT_JS = """(() => {
  if (window.__xprofDataTableHooked) return;
  window.__xprofDataTableHooked = true;
  const sym = Symbol('dataTable');
  Object.defineProperty(Object.prototype, 'dataTable', {
    configurable: true,
    get() {
      return this[sym];
    },
    set(val) {
      ...
      if (val && this.update && typeof this.update.emit === 'function') {
        if (!this.update.__xprofWrapped) {
          const origEmit = this.update.emit.bind(this.update);
          this.update.emit = (...args) => {
            this.__xprofEmitted = true;
            return origEmit(...args);
          };
          this.update.__xprofWrapped = true;
        }
        this.__xprofEmitted = false;
        Promise.resolve().then(() => {
          if (!this.__xprofEmitted) {
            try {
              this.update.emit();
            } catch (e) {}
          }
...
class NetworkRecorder:
  ...
  def __init__(self, page: typing.Any):
    ...
    if hasattr(page, "add_init_script"):
      page.add_init_script(_CHART_DATA_PROVIDER_INIT_JS)
...
_ANGULAR_IS_STABLE_JS = """() => {
  ...
  const hasVisibleIframe = Array.from(document.querySelectorAll('iframe')).some(
      f => f.offsetParent !== null && f.getBoundingClientRect().width > 0
  );
  if (hasVisibleIframe) {
...
def _wait_for_nested_documents(page: typing.Any) -> bool:
  ...
  while time.monotonic() < deadline:
    try:
      ...
      needs_canvas = "trace_viewer" in page.url
      states = []
      painted = True
      for frame in frames:
        state = frame.evaluate(_NESTED_DOC_STATE_JS)
        states.append(state)
        if not _nested_doc_is_painted(
            needs_canvas or "trace_viewer" in frame.url,
            state,
            getattr(frame, "url", ""),
        ):
          painted = False
...
def _wait_for_charts_to_render(page: typing.Any) -> bool:
  ...
  1. While visible charts exist and none of them have drawn yet (drawn == 0),
     the Google Charts loader or tool response is still in flight, so the wait
     must not settle.
  2. Once at least one chart has drawn (drawn > 0), any remaining blank charts
     are either finishing in the same draw batch or bound to an empty category
     that will never draw (such as unused rematerialization charts in HLO Op
     Stats), so the wait finishes as soon as the (drawn, pending) pair holds
  ...
  while time.monotonic() < deadline:
    ...
    if not pending:
      return True
    if drawn > 0 and counts == previous:
      stable_count += 1
