...
class SxsDiffEngine:
  ...
  def sanitize_dom(self, html: str) -> str:
    ...
    cleaned = re.sub(
        r' id="mat-(tab-label|tab-content|select|option)-[0-9]+(-[0-9]+)?"',
        "",
        cleaned,
    )
    cleaned = re.sub(
    ...
    cleaned = re.sub(
        r' aria-controls="mat-tab-content-[0-9]+-[0-9]+"', "", cleaned
    )
    cleaned = re.sub(r' aria-owns="mat-select-[0-9]+-panel"', "", cleaned)
    return "\n".join(
  ...
  def evaluate_waypoint(
    ...
    if visual.composite_png_bytes:
      hasher.update(visual.composite_png_bytes)
