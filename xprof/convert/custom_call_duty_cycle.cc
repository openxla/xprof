...
bool IsCustomCallOffDuty(int64_t flops, int64_t model_flops, double flops_v2,
  ...
  return (flops <= 0) && (model_flops <= 0) &&
         (flops_v2 <= 0.0 || ApproximatelyEqual(flops_v2, 0.0)) &&
         (model_flops_v2 <= 0.0 || ApproximatelyEqual(model_flops_v2, 0.0)) &&
         !has_ici;
}

bool IsCustomCallEventOffDuty(const tsl::profiler::XEventVisitor& event) {
  std::optional<tsl::profiler::XStatVisitor> category =
      event.GetEventOrMetadataStat(tsl::profiler::StatType::kHloCategory);
  ...
  int64_t flops =
      event.GetEventOrMetadataStat(tsl::profiler::StatType::kFlops, 0);
  if (flops > 0) return false;

  int64_t model_flops =
      event.GetEventOrMetadataStat(tsl::profiler::StatType::kModelFlops, 0);
  if (model_flops > 0) return false;

  bool has_ici =
      event.GetEventOrMetadataStat(tsl::profiler::StatType::kUsesIci, 0) > 0;
  if (has_ici) return false;
