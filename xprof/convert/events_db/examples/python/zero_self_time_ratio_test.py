...
# ==============================================================================
"""Tests for zero_self_time_ratio example binary."""
...
from xprof.convert.events_db.examples.python import zero_self_time_ratio
...
class ZeroSelfTimeRatioTest(absltest.TestCase):
  ...
  def test_main_runs_with_flags(self) -> None:
    ...
    with (
      ...
      zero_self_time_ratio.main(["zero_self_time_ratio"])

    with self.subTest("stdout_contains_expected_output"):
      output = f.getvalue()
      self.assertIn(
          "Calculating ratio of events with zero self_time_ns", output
      )
      ...
      self.assertIn("Zero self_time_ns ratio: 0.5000 (50.00%)", output)
  ...
  def test_too_many_args_raises(self) -> None:
    with flagsaver.flagsaver(
      ...
      zero_self_time_ratio.main(["zero_self_time_ratio", "unexpected_arg"])
