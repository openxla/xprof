...
TEST(FlatOpMetricsToRecordTest, GigaFlopsPerSecondPerCoreNormalizedOnDvfs) {
  FlatOpMetrics metrics;
  metrics.set_time_ps(2000);
  metrics.set_normalized_time_ps(1000);
  metrics.set_flops_v2(100);

  // Raw GigaFlopsPerSecondPerCore = 100 / 2.0 = 50.0
  EXPECT_DOUBLE_EQ(GigaFlopsPerSecondPerCore(metrics), 50.0);

  // DVFS scaling ratio = time_ps / normalized_time_ps = 2000 / 1000 = 2.0
  // Normalized GigaFlopsPerSecondPerCore = 50.0 * 2.0 = 100.0
  EXPECT_DOUBLE_EQ(GigaFlopsPerSecondPerCoreNormalizedOnDvfs(metrics), 100.0);
}
