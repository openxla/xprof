...
void UpdateEventTimestamps(
  ...
  if (request_events_map != nullptr) {
    if (auto request_events =
            tsl::gtl::FindOrNull(*request_events_map, group_id)) {
    ...
    for (const int64_t parent_group_id : group_metadata->parents) {
      if (auto parent_request_events =
              tsl::gtl::FindOrNull(*request_events_map, parent_group_id)) {
...
void UpdateBatchEvents(const GroupMetadataMap& group_metadata_map,
  ...
  if (auto batch_events = tsl::gtl::FindOrNull(*batch_events_map, group_id)) {
...
void InitializeRequestEvents(
  ...
  if (is_batching_request) {
    ...
    const std::string* children_model_id =
        tsl::gtl::FindOrNull(model_id_map, children_group_id);
    ...
    const std::string* model_id = tsl::gtl::FindOrNull(model_id_map, group_id);
    if (model_id) {
  ...
  for (const auto& [_, events] : host_events_by_type) {
    for (const auto& event : events) {
      ...
      if (optional_group_id.has_value()) {
        if (RequestEvents* request = tsl::gtl::FindOrNull(
...
void UpdateTpuDataTransferEventsInTpuSystem(
  ...
  auto build_events =
      [&](const HostEventType event_type,
        ...
        if (const auto* events =
                tsl::gtl::FindOrNull(host_events_by_type, event_type)) {
  ...
  build_events(data_transfer_end_event, [&](uint64_t id,
                                            const XEventVisitor* end_event) {
    if (auto* value = tsl::gtl::FindOrNull(events_per_transfer, id)) {
...
void BuildTPUDeviceEvents(const std::vector<XPlane*>& device_traces,
  ...
  for (const int64_t data_transfer_type : kDataTransferTypes) {
    if (const auto* data_transfer_events =
            tsl::gtl::FindOrNull(host_events_by_type, data_transfer_type)) {
  ...
  for (const int64_t tpu_execute_type : kTPUExecuteTypes) {
    if (const auto* tpu_execute_events =
            tsl::gtl::FindOrNull(host_events_by_type, tpu_execute_type)) {
  ...
  for (const int64_t tpu_program_launch_type : kTPUProgramLaunchTypes) {
    if (const auto* tpu_program_launch_events = tsl::gtl::FindOrNull(
            host_events_by_type, tpu_program_launch_type)) {
  ...
  // host postprocessing.
  if (const auto* tpu_complete_callback_events = tsl::gtl::FindOrNull(
...
ModelIdMap InitializeModelIdMap(
  ...
  for (const int64_t event_type : kModelIdRequestTypes) {
    auto event_list = tsl::gtl::FindOrNull(host_events_by_type, event_type);
...
void BuildRequestEventsMap(const std::vector<XPlane*>& device_traces,
  ...
  if (const auto* process_batch_events = tsl::gtl::FindOrNull(
          host_events_by_type, HostEventType::kProcessBatch)) {
  ...
  for (const int64_t request_type : request_types) {
    if (const auto* request_events =
            tsl::gtl::FindOrNull(host_events_by_type, request_type)) {
  ...
  for (const int64_t schedule_type : kScheduleEventTypes) {
    if (const auto* schedule_events =
            tsl::gtl::FindOrNull(host_events_by_type, schedule_type)) {
...
void BuildBatchEventsMap(const std::vector<XPlane*>& device_traces,
  ...
  for (const int64_t padding_event_type : kPaddingEventTypes) {
    if (const auto* padding_events =
            tsl::gtl::FindOrNull(host_events_by_type, padding_event_type)) {
      for (const XEventVisitor& padding_event : *padding_events) {
        ...
        BatchEvents* batch_events =
            tsl::gtl::FindOrNull(*batch_events_map, group_id);
  ...
  for (auto& [batch_id, batch_events] : *batch_events_map) {
    ...
    if (!batch_detail.related_request_ids().empty()) {
      ...
      const RequestEvents* request_events =
          tsl::gtl::FindOrNull(*request_events_map, first_request_id);
      if (request_events) {
...
void GenerateRequestAndBatchDelay(RequestEventsMap* request_events_map,
  ...
  for (auto& [request_id, request_event] : *request_events_map) {
    ...
    for (const int64_t batch_id : request_event.related_batch_ids) {
      const auto* batch_events =
          tsl::gtl::FindOrNull(*batch_events_map, batch_id);
      if (!batch_events) continue;
  ...
  for (auto& [batch_id, batch_events] : *batch_events_map) {
    ...
    for (const int64_t request_id : batch_detail.related_request_ids()) {
      const auto* request_events =
          tsl::gtl::FindOrNull(*request_events_map, request_id);
...
void GenerateTensorDetails(
  ...
  for (const int64_t tensor_detail_event_type : kTensorDetailEventTypes) {
    if (const auto* tensor_detail_events = tsl::gtl::FindOrNull(
            host_events_by_type, tensor_detail_event_type)) {
  ...
  auto get_tensor_pattern_index = [&tensor_patterns](
                                      const std::string& tensor_pattern) {
    if (int* index = tsl::gtl::FindOrNull(tensor_patterns, tensor_pattern)) {
      return *index;
    }
...
void ParseTfstreamzForBatchingParameter(
  ...
  for (const auto& [model_id_tfstreamz, params] : model_params) {
    if (const std::vector<absl::string_view>* model_ids_session_run =
            tsl::gtl::FindOrNull(model_id_map, model_id_tfstreamz)) {
