...
TEST(InferenceStatsGroupingTest,
     UsesBatchDeviceTimeForSplitRequests) {
  InferenceStats inference_stats = ParseTextProto<InferenceStats>(R"pb(
                                     inference_stats_per_host {
                                       key: 0
                                       value {
  ...
  // Batch size 1 bucket should have device_time_ps = 1000 (not 6000).
  ...
  EXPECT_EQ(bs1.aggregated_batch_result().batch_id(), 11);

  // Batch size 8 bucket should have device_time_ps = 5000 (not 6000).
  ...
  EXPECT_EQ(bs8.aggregated_batch_result().batch_id(), 10);
  EXPECT_NEAR(bs8.aggregated_batch_result().batching_efficiency(), 7.0 / 8.0,
              1e-6);
}

TEST(InferenceStatsGroupingTest, PropagatesProgramIdsToAggregatedBatchDetails) {
  InferenceStats inference_stats = ParseTextProto<InferenceStats>(R"pb(
                                     inference_stats_per_host {
                                       key: 0
                                       value {
  ...
  EXPECT_THAT(bs8.aggregated_batch_result().program_ids(),
              ::testing::UnorderedElementsAre(11111, 22222));
  EXPECT_THAT(model_stats.aggregated_batch_detail().program_ids(),
              ::testing::UnorderedElementsAre(11111, 22222));
}
