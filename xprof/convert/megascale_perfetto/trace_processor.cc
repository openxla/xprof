...
int ExtractMultiRecvCountFromLongName(const Event& event,
                                      const XprofTrace& trace) {
  int count = 1;  // default
  absl::string_view long_name;
  if (!FindArgString(event, trace, "long_name", &long_name)) {
  ...
  static constexpr LazyRE2 kMultiRecvCountRe = {
      R"re(_xla_multi_recv_count="(\d+)")re"};
  // If successful, it will overwrite `count`.
  RE2::PartialMatch(long_name, *kMultiRecvCountRe, &count);
  return count;
}

int64_t ExtractHloId(absl::string_view name) {
  static constexpr LazyRE2 kHloId = {R"re([a-zA-Z_-]+\.(\d+))re"};
  int64_t id;
  if (RE2::FullMatch(name, *kHloId, &id)) {
    return id;
  }
  return -1;
}
...
void TraceProcessor::ResolveFlows() {
  ...
  absl::flat_hash_map<std::string, int> recv_hlo_to_multi_count;
  ...
  auto push_flow = [&](FlowQueueMap& queue, const std::string& key,
    ...
    event.flows.push_back({fid, FlowDirection::kSource});
    event.args.push_back({trace_.string_table.Intern("flow_out"), fid});
    queue.Push(key, fid);
  };
  ...
  auto pop_flow = [&](FlowQueueMap& queue, const std::string& key,
    ...
    if (fid != -1) {
      event.args.push_back({trace_.string_table.Intern("flow_in"), fid});
      event.flows.push_back({fid, FlowDirection::kSink});
    }
    return fid;
  };
  ...
  visit_tpu_ops([&](int64_t tpu_id, Track& track, Event& event) {
    ...
    if (is_send) {
      ...
      recv_hlo_to_multi_count[hlo_key] =
          ExtractMultiRecvCountFromLongName(event, trace_);
  ...
  visit_tpu_ops([&](int64_t tpu_id, Track& track, Event& event) {
    ...
    if (is_send_done) {
      ...
      // For H2D to recv-done, we may have multiple.
      int multi_count =
          tsl::gtl::FindWithDefault(recv_hlo_to_multi_count, hlo_key, 1);
