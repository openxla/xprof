...
void AddHloStatsRecord(HloStatsDatabase& hlo_stats_db, const OpMetrics* metrics,
  ...
  if (metrics->children().metrics_db_size() > 0) {
    ...
    for (const OpMetrics* child_metrics :
      ...
      AddHloStatsRecord(hlo_stats_db, child_metrics, op_stats,
                        child_prev_record, child_total_device_time_us,
                        metrics->name());
    }
  }
}
...
HloStatsDatabase ConvertOpStatsToHloStats(const OpStats& op_stats) {
  ...
  for (const OpMetrics* metrics :
    ...
    AddHloStatsRecord(hlo_stats_db, metrics, op_stats, prev_record,
