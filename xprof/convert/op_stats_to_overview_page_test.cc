...
TEST(OpStatsToOverviewPageTest, ComputeTpuAnalysisResult_FlatMetrics) {
  OpStats op_stats = ParseTextProtoOrDie(R"pb(
    flat_device_op_metrics_db {
      busy_time_ps: 800000
      idle_time_ps: 200000
      busy_time_high_confidence_ps: 900000
      idle_time_high_confidence_ps: 100000
      total_time_ps: 1000000
      total_op_time_ps: 800000
    }
  ...
  // High-confidence values are distinct from the regular ones above, so these
  // fail if the wrong pair of fields is read.
  EXPECT_DOUBLE_EQ(analysis.device_duty_cycle_high_confidence_percent(), 90.0);
  EXPECT_DOUBLE_EQ(analysis.idle_time_high_confidence_ps(), 100000);
  EXPECT_DOUBLE_EQ(
      analysis.flop_rate_utilization_relative_to_roofline_percent(), 50.0);
  EXPECT_DOUBLE_EQ(
      analysis.memory_bw_utilization_relative_to_hw_limit_percent(), 50.0);
