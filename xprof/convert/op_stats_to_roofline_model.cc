...
void ConvertOpMetricsToRooflineModelRecordImpl(
    ...
    bool apply_time_scale_multiplier, RooflineModelRecord* record) {
...
RooflineModelRecord ConvertOpMetricsToRooflineModelRecord(
  ...
  ConvertOpMetricsToRooflineModelRecordImpl(
...
RooflineModelRecord GenerateRooflineModelProgramRecordImpl(
  ...
  using MetricsT =
      std::conditional_t<std::is_same_v<MetricsDbT, FlatOpMetricsDb>,
                         FlatOpMetrics, OpMetrics>;
  MetricsT program_metrics;
  if constexpr (std::is_same_v<MetricsDbT, FlatOpMetricsDb>) {
  ...
  const auto& metrics_list = [&]() -> const auto& {
    if constexpr (std::is_same_v<MetricsDbT, FlatOpMetricsDb>) {
  ...
  for (const MetricsT& metrics : metrics_list) {
    ...
    if constexpr (std::is_same_v<MetricsDbT, FlatOpMetricsDb>) {
      // Skipping sparse core metrics to have equivalence with OpMetricsDb.
    ...
    if constexpr (std::is_same_v<MetricsDbT, FlatOpMetricsDb>) {
      FlatOpMetricsDbCombiner::CombineMemoryAccessedBreakdown(
...
RooflineModelRecord GenerateRooflineModelProgramRecord(
    ...
    uint32_t step_num, const RooflineModelDatabase& roofline_model_db,
    bool include_infeed_outfeed, bool apply_time_scale_multiplier) {
  return GenerateRooflineModelProgramRecordImpl(
...
ConvertOpMetricsDbToRooflineModelRecordsImpl(
    const OpStats& op_stats, const MetricsDbT& db, RecordType record_type,
    uint32_t step_num, const RooflineModelDatabase& roofline_model_db,
    bool include_infeed_outfeed, bool apply_time_scale_multiplier) {
  using MetricsT =
...
ConvertFlatOpMetricsDbToRooflineModelRecords(
    ...
    uint32_t step_num, const RooflineModelDatabase& roofline_model_db,
    bool include_infeed_outfeed, bool apply_time_scale_multiplier) {
  return ConvertOpMetricsDbToRooflineModelRecordsImpl(
      op_stats, db, record_type, step_num, roofline_model_db,
      include_infeed_outfeed, apply_time_scale_multiplier);
}
...
RooflineModelDatabase InitializeRooflineModelDatabaseFromOpStats(
  ...
  if (hardware_type == tensorflow::profiler::HardwareType::UNKNOWN_HARDWARE) {
    hardware_type = tensorflow::profiler::ParseHardwareType(
        op_stats.run_environment().device_type());
  }
  ...
  SetTimeScaleMultiplier(op_stats, &roofline_model_db, use_flat_op_metrics_db);
...
RooflineModelDatabase ConvertOpStatsToRooflineModel(
  ...
  if (hardware_type == HardwareType::UNKNOWN_HARDWARE) {
    hardware_type = tensorflow::profiler::ParseHardwareType(
        op_stats.run_environment().device_type());
  }
