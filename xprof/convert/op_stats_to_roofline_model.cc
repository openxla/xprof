  ...
  *record.mutable_source_info() = metrics.source_info();
  ...
  if (apply_time_scale_multiplier) {
    double measured_flop_rate_normalized =
        GigaFlopsPerSecondPerCoreNormalizedOnTimeScaleFactor(metrics);
...
RooflineModelDatabase InitializeRooflineModelDatabaseFromOpStats(
  ...
  //     op_stats.device_op_metrics_db().root_op_total_time_ps(),
  //     op_stats.device_op_metrics_db().root_op_total_normalized_time_ps()));
  roofline_model_db.set_time_scale_multiplier(
      op_stats.device_op_metrics_db().dvfs_time_scale_multiplier());
