...
// This struct and function are copied from the .cc file for test purposes.
...
TEST_F(StreamingTraceViewerProcessorTest, ReduceWithEventNameSearch) {
  XSpace space = CreateTestXSpace(2);
  absl::flat_hash_map<std::string, XSpace> host_xspaces = {{"host1", space}};
  TF_ASSERT_OK_AND_ASSIGN(SessionSnapshot snapshot,
                          CreateSnapshot(host_xspaces));
  ...
  ToolOptions tool_options;
  tool_options["event_name"] = "SessionRun";  // Updated name
  tool_options["start_time_ms"] = "1100.0";
  tool_options["duration_ms"] = "100.0";
