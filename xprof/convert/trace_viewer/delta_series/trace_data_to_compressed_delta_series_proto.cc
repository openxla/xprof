...
TraceMetadata DeltaSeriesProtoConverter::GetTraceMetadata() const {
  TraceMetadata metadata;
  for (const auto& [device_id, device] : trace_->devices()) {
    Process* process = metadata.add_processes();
    ...
    for (const auto& [resource_id, resource] : device.resources()) {
      Thread* thread = process->add_threads();
...
void DeltaSeriesProtoConverter::PopulateTraceEventMetadata(
    const TraceEvent& event, TraceEventMetadata* metadata) {
...
void DeltaSeriesProtoConverter::AddCompleteEventTrack(
  ...
  TraceEventSeries* series = response->add_complete_events();
  TraceEventSeriesMetadata* series_metadata = series->mutable_metadata();
...
void DeltaSeriesProtoConverter::AddAsyncEventTrack(
    ...
    TraceDataResponse* response) {
  TraceEventSeries* series = response->add_async_events();
  TraceEventSeriesMetadata* series_metadata = series->mutable_metadata();
  series_metadata->set_process_id(pid);
...
void DeltaSeriesProtoConverter::AddCounterEventTrack(
    uint32_t pid, absl::string_view name, const TraceEventTrack& events,
    TraceDataResponse* response) {
  TraceEventSeries* series = response->add_counter_events();
  TraceEventSeriesMetadata* series_metadata = series->mutable_metadata();
  series_metadata->set_process_id(pid);
  series_metadata->set_name_ref(MaybeInternString(name));
  ...
  for (const auto* event : events) {
    ...
    TraceEventMetadata* event_metadata = series->add_event_metadata();
