...
void PrefixTrie::Insert(absl::string_view key, absl::string_view id) {
  ...
  for (const char c : key) {
    if (node->children.contains(c)) {
      ...
      node->children[c] = std::make_unique<PrefixTrieNode>(node->key + c);
      node = node->children[c].get();
    }
  }
...
void IterateTrieAndSaveToLevelDbTable(PrefixTrieNode* node,
  ...
  builder.Add(node->key, proto.SerializeAsString());
  for (const auto& [c, child] : node->children) {
    IterateTrieAndSaveToLevelDbTable(child.get(), builder);
  }
}
...
absl::Status PrefixTrie::SaveAsLevelDbTable(
  ...
  IterateTrieAndSaveToLevelDbTable(root_.get(), builder);
...
absl::StatusOr<std::vector<PrefixSearchResult>> LoadTrieAsLevelDbTableAndSearch(
  ...
  while (iterator->Valid()) {
    ...
    if (key.starts_with(prefix)) {
      PrefixTrieNodeProto proto;
      proto.ParseFromString(iterator->value());
      if (!proto.terminal_key_ids().empty()) {
        results.push_back(
            {.key = key,
             .terminal_key_ids = {proto.terminal_key_ids().begin(),
