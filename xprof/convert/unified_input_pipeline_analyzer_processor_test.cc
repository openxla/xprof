...
TEST(UnifiedInputPipelineAnalyzerProcessorTest, MinimalTest) {
  ...
  std::unique_ptr<UnifiedProfileProcessor> processor =
      UnifiedProfileProcessorFactory::GetInstance().Create(
          "input_pipeline_analyzer", options);
  ASSERT_NE(processor, nullptr);

  std::string session_dir = tsl::io::JoinPath(
      testing::TempDir(), "unified_input_pipeline_analyzer_processor_test");
...
TEST(UnifiedInputPipelineAnalyzerProcessorTest, FlatOpMetricsDbTest) {
  RegisterUnifiedToolRegistrations();
  ToolOptions options;
  ...
  std::string session_dir = tsl::io::JoinPath(
      testing::TempDir(),
      "unified_input_pipeline_analyzer_processor_flat_test");
  ASSERT_OK(tsl::Env::Default()->RecursivelyCreateDir(session_dir));
  std::string xspace_path =
      tsl::io::JoinPath(session_dir, "test_host.xplane.pb");
  XSpace dummy_space;
