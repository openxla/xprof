...
TEST_F(UnifiedOverviewPageProcessorTest, ProcessCombinedOpStatsTrainingTest) {
  ...
  combined_op_stats.mutable_run_environment()->set_is_training(true);

  ASSERT_OK(overview_processor->ProcessCombinedOpStats(
      session_snapshot, combined_op_stats, options_));
...
TEST_F(UnifiedOverviewPageProcessorTest, ProcessCombinedOpStatsInferenceTest) {
  ...
  tensorflow::profiler::OpStats combined_op_stats;
  combined_op_stats.mutable_run_environment()->set_device_type("TPU");
  combined_op_stats.mutable_run_environment()->set_is_training(false);
