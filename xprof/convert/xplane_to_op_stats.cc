...
absl::StatusOr<OpStats> ConvertXSpaceToOpStats(const XSpace& space,
  ...
  }
  {
    ...
    auto op_metrics_cleanup = absl::MakeCleanup([&all_op_metrics_dbs,
      ...
      if (use_flat_op_metrics_db) return;
    ...
    auto flat_op_metrics_cleanup = absl::MakeCleanup(
        [&all_flat_op_metrics_dbs, &flat_op_metrics_db_combiner,
         &hlo_module_map, &op_stats, use_flat_op_metrics_db]() {
          if (!use_flat_op_metrics_db) return;
  ...
  if (is_tpu) {
    if (use_flat_op_metrics_db) {
      ...
      op_metrics_db.set_busy_time_ps(duty_cycle_combiner.
                                     GetTotalActiveTimePs());
    }
  }
  ...
  if (options.generate_step_db) {
    if (is_tpu) {
      ...
      *op_stats.mutable_step_db() = ConvertStepEventsToStepDb(
      ...
      if (use_flat_op_metrics_db) {
        *op_stats.mutable_flat_device_op_metrics_db()
             ->mutable_precision_stats() = ComputePrecisionStats(step_events);
      } else {
        *op_stats.mutable_device_op_metrics_db()
             ->mutable_precision_stats() = ComputePrecisionStats(step_events);
      }
