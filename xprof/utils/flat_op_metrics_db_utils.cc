...
FlatOpMetrics* FlatOpMetricsDbBuilder::LookupOrInsertNewFlatOpMetrics(
  ...
  ModuleMap& module_map = op_metrics_map_[hlo_module_id];
  if (auto it = module_map.find(hlo_name); it != module_map.end()) {
    return it->second;
  }
  FlatOpMetrics* op_metrics = db_->add_op_instances();
  op_metrics->set_hlo_module_id(hlo_module_id);
  op_metrics->set_hlo_name(hlo_name);
...
XEventsFlatOpMetricsDbBuilder::GetFlatOpKeyFromXEvent(
  ...
  tsl::profiler::XEventMetadataVisitor metadata = event.Metadata();
  if (auto stat = metadata.GetStat(StatType::kProgramId); stat.has_value()) {
    op_key.program_id = stat->IntOrUintValue();
  }
  if (auto stat = metadata.GetStat(StatType::kSymbolId); stat.has_value()) {
...
FlatOpMetricsDb XEventsFlatOpMetricsDbBuilder::FinalizeSorted() {
  ...
  for (const auto& [op_id, op_metrics] : metrics_map) {
    total_op_time_ps += op_metrics.self_time_ps();
    normalized_total_op_time_ps +=
        op_metrics.self_time_ps() *
        tsl::profiler::SafeDivide(op_metrics.normalized_time_ps() * 1.0,
                                  op_metrics.time_ps());
    *db.add_op_instances() = op_metrics;
  }
