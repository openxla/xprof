...
TEST(HostInfeedEnqueueRatioTest, Basic) {
  FlatOpMetricsDb db;
  // Denominator is 0
  EXPECT_EQ(HostInfeedEnqueueRatio(db), std::nullopt);

  db.set_total_host_infeed_enq_start_timestamp_ps_diff(0);
  EXPECT_EQ(HostInfeedEnqueueRatio(db), std::nullopt);

  // Denominator > 0
  db.set_total_host_infeed_enq_start_timestamp_ps_diff(100);
  db.set_total_host_infeed_enq_duration_ps(50);
  std::optional<double> ratio = HostInfeedEnqueueRatio(db);
