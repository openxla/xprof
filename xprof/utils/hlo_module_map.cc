...
class MatmulOpEstimator : public OperationCostEstimator {
 public:
  OperationCost Estimate(mlir::Operation* op) const override {
    ...
    // auto lhs_st = mlir::dyn_cast<mlir::ShapedType>
    // (op->getOperand(0).getType());
    // auto rhs_st = mlir::dyn_cast<mlir::ShapedType>
    // (op->getOperand(1).getType());
    // auto result_st =
    //     mlir::dyn_cast<mlir::ShapedType>(op->getResult(0).getType());

    // if (!lhs_st || !lhs_st.hasStaticShape() || !rhs_st ||
    //     !rhs_st.hasStaticShape() || !result_st ||
    // !result_st.hasStaticShape()) {
    ...
    //     LOG(WARNING) << "dimension_numbers
    // found but has no contracting dims.";
    //     return cost;
    //   }
