...
class MatmulOpEstimator : public OperationCostEstimator {
 public:
  OperationCost Estimate(mlir::Operation* op) const override {
    std::string op_str;
    OperationCost cost;
    auto matmul_op = mlir::dyn_cast<mlir::tpu::MatmulOp>(op);

    if (!matmul_op) {
      LOG(WARNING) << "Matmul Casting Failed";
      return cost;
    }
    ...
    auto lhs_st = mlir::dyn_cast<mlir::ShapedType>(op->getOperand(0).getType());
    auto rhs_st = mlir::dyn_cast<mlir::ShapedType>(op->getOperand(1).getType());
    auto result_st =
    ...
    if (!lhs_st || !lhs_st.hasStaticShape() || !rhs_st ||
      ...
      return cost;  // Need static shapes for cost estimation.
    }

    uint64_t contracting_size_prod = 1;
    auto dims =
        op->getAttrOfType<mlir::tpu::DotDimensionNumbersAttr>(
        "dimension_numbers");
    if (dims) {
      LOG(INFO) << "Found dimension_numbers attribute.";
      auto contracting_dims = dims.getLhsContractingDims();
      if (contracting_dims.empty()) {
        LOG(WARNING) << "dimension_numbers found but has no contracting dims.";
        return cost;
      }
      for (int64_t d : contracting_dims) {
        if (d >= lhs_st.getRank()) return cost;  // Invalid dim index.
        contracting_size_prod *= lhs_st.getShape()[d];
      }
    } else {
      LOG(INFO) << "No dimension_numbers attribute, assuming contraction on "
                   "last dim of LHS.";
      if (lhs_st.getRank() < 1) return cost;
      // Fallback for standard matmul: contract last dim of LHS.
      contracting_size_prod = lhs_st.getShape()[lhs_st.getRank() - 1];
    }

    cost.flops = 2 * result_st.getNumElements() * contracting_size_prod;
    const auto& data_layout = mlir::DataLayout::closest(op);
    cost.bytes_consumed = data_layout.getTypeSize(op->getResult(0).getType());
...
// Estimator for store operations like vector.store or tpu.store.
