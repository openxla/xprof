...
absl::string_view GetFileFromStackFrameIndex(
    const xla::StackFrameIndexProto& stack_frame_index, int32_t frame_id) {
  if (frame_id > 0 && frame_id <= stack_frame_index.stack_frames_size()) {
    const auto& frame = stack_frame_index.stack_frames(frame_id - 1);
    if (const auto file_location_id = frame.file_location_id();
        file_location_id > 0 &&
        file_location_id <= stack_frame_index.file_locations_size()) {
      ...
      if (const auto file_name_id = file_location.file_name_id();
          file_name_id > 0 &&
          file_name_id <= stack_frame_index.file_names_size()) {
        return stack_frame_index.file_names(file_name_id - 1);
      }
    }
  }
  return "";
...
int32_t GetLineFromStackFrameIndex(
    const xla::StackFrameIndexProto& stack_frame_index, int32_t frame_id) {
  if (frame_id > 0 && frame_id <= stack_frame_index.stack_frames_size()) {
    const auto& frame = stack_frame_index.stack_frames(frame_id - 1);
    if (const auto file_location_id = frame.file_location_id();
...
OpSourceInfo GetSourceInfo(const xla::HloInstructionProto& instr,
  ...
  absl::string_view source_file = instr.metadata().source_file();
  int32_t source_line = instr.metadata().source_line();
  if (source_file.empty()) {
    source_file = GetFileFromStackFrameIndex(stack_frame_index,
                                             instr.metadata().stack_frame_id());
    source_line = GetLineFromStackFrameIndex(stack_frame_index,
                                             instr.metadata().stack_frame_id());
  }
  return GetSourceInfo(source_file, source_line, stack_frame);
}
...
OpSourceInfo GetSourceInfo(const xla::HloInstruction& instr) {
  ...
  // instr.metadata().source_file() is a persistent string.
