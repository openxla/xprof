...
void DeviceOpMetricsDbBuilder::EnterOp(const OpIdentifier& op_id,
  ...
  const int64_t flops = std::max<int64_t>(
      0, (perf_info != nullptr && perf_info->DeviceFlops() > 0)
             ? perf_info->DeviceFlops()
             : event_data.flops);
  const int64_t bytes_accessed = std::max<int64_t>(
      0, (perf_info != nullptr && perf_info->bytes_accessed() > 0)
  ...
  const int64_t model_flops =
      std::max<int64_t>(0, (perf_info != nullptr && perf_info->ModelFlops() > 0)
...
void DeviceFlatOpMetricsDbBuilder::EnterOp(const OpIdentifier& op_id,
  ...
  const auto* perf_info = event_data.perf_info;
  const int64_t flops = std::max<int64_t>(
      0, perf_info != nullptr ? perf_info->DeviceFlops() : event_data.flops);
  const int64_t bytes_accessed =
      std::max<int64_t>(0, perf_info != nullptr ? perf_info->bytes_accessed()
                                                : event_data.bytes_accessed);
  const int64_t model_flops =
      std::max<int64_t>(0, perf_info != nullptr ? perf_info->ModelFlops()
                                                : event_data.model_flops);
