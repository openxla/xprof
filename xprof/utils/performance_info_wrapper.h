...
//
// Does not mutate the underlying PerformanceInfo proto (including const
// CompilerMetadata-sourced protos). Cost accessors apply ValidHloCost so the
// HloCostAnalysis "no cost" sentinel (-1) is reported as 0; other values
// (including large positives and non-sentinel negatives) pass through.
class PerformanceInfoWrapper {
 public:
  ...
  // Bytes accessed for the instruction; -1 (unknown) is mapped to 0.
  int64_t BytesAccessed() const {
    return ValidHloCost(performance_info_->bytes_accessed());
  }

  // Number of flops required for the hlo_instruction; -1 (unknown) → 0.
